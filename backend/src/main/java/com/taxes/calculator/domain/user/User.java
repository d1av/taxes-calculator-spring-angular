...
public class User extends AggregateRoot<UserID> {
    ...
    public static User newUser(final String aName,
	    final String aPassword, final Boolean aActive) {
	final var anId = UserID.unique();
	final var now = InstantUtils.now();
	final var deletedAt = aActive ? null : now;
	return new User(anId, aName, aPassword, aActive,
		new HashSet<>(), now, now, deletedAt);
    }

    public static User newUser(final UserID anId, final String aName,
	    final String aPassword, final Boolean aActive) {
	final var now = InstantUtils.now();
	final var deletedAt = aActive ? null : now;
	return new User(anId, aName, aPassword, aActive,
	...
	return new User(user.id, user.name, user.password,
		user.active, user.roles, user.createdAt,
	...
	this.roles = new HashSet<>(!Objects.isNull(roles) ? roles
	...
	if (notification.hasError()) {
	    throw new NotificationException(
		    "Failed to validate Aggregate User",
		    notification);
	}
    }
